...
with col1:
    ...
    gender = st.selectbox("Jenis Kelamin", ["Male", "Female", "Prefer not to say"], help="Jenis kelamin pelanggan.")
    marital_status = st.selectbox("Status Pernikahan", ["Single", "Married", "Prefer not to say"], help="Status pernikahan pelanggan.")
    occupation = st.selectbox("Pekerjaan", ["Student", "Employee", "Self Employeed", "House wife"], help="Pekerjaan pelanggan.")

with col2:
    # Mapping untuk Monthly Income (HARUS SAMA dengan preprocessing di notebook)
    monthly_income_options = ["No Income", "Below Rs.10000", "10001 to 25000", "25001 to 50000", "More than 50000"]
    monthly_income = st.selectbox("Pendapatan Bulanan", monthly_income_options, help="Estimasi pendapatan bulanan pelanggan.")
    monthly_income_mapped = {
        "No Income": 0, "Below Rs.10000": 1, "10001 to 25000": 2,
        "25001 to 50000": 3, "More than 50000": 4
    }[monthly_income]

    # Mapping untuk Educational Qualifications (HARUS SAMA dengan preprocessing di notebook)
    edu_qual_options = ["School", "Graduate", "Post graduate", "Ph.d"]
    educational_qualifications = st.selectbox("Kualifikasi Pendidikan", edu_qual_options, help="Tingkat pendidikan terakhir pelanggan.")
    edu_qual_mapped = {
...
# Buat DataFrame dari input pengguna. PENTING: Urutan kolom harus sama dengan X_train asli!
input_data = pd.DataFrame([{
    'Age': age,
    'Gender': gender,
    'Marital Status': marital_status,
    'Occupation': occupation,
    'Monthly Income': monthly_income_mapped,
    'Educational Qualifications': edu_qual_mapped,
    'Family size': family_size,
    'Feedback': feedback
}])

st.write("---") # Garis pemisah sebelum tombol prediksi

# --- Tombol Prediksi di area utama untuk menyeimbangkan layout ---
col_btn1, col_btn2, col_btn3 = st.columns([1, 2, 1]) # Proporsi 1:2:1 untuk menengahkan tombol

with col_btn2: # Tombol akan berada di kolom tengah
...
st.markdown("---")
st.markdown("Aplikasi dibuat oleh **Muthia Rahmatun Nisa'**") # Ganti dengan nama Anda
